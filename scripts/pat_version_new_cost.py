...
from orquestra.drug.discovery.docking.utils import process_molecule, process_molecules
...
@click.option("--dropout", type=float, default=0.35912916665692707)
@click.option("--n-epochs", type=int, default=1)
@click.option("--gradient-accumulation-steps", type=int, default=5)
@click.option("--batch-size", type=int, default=64)
@click.option("--learning-rate", type=float, default=0.0006225117554833231)
...
def main(
    ...
    for epoch in range(n_epochs):
        with tqdm.tqdm(total=len(selfies_dl)) as prog_bar:
            ...
            for step, tensor_batch in enumerate(selfies_dl):
                ...
                total_loss = compute_transformer_loss(
                    model, tensor_batch, pad_index=selfies.pad_index
                )
        ...
        smiles_dic = process_molecules(ligands_before, parallel_cpus=200)
        pass_list = [
            smiles for smiles, status in smiles_dic.items() if status == "PASS"
        ]
        sr_rate = len(pass_list) / len(smiles_dic)
