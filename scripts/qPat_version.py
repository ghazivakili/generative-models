...
if len(sys.argv)>2:
    ...
    if str(sys.argv[6]) == "sim":
        ...
        backend_sim = False
    # print(f"prior_name:{prior_name},prior_size:{prior_size},DEVICE:{DEVICE},cuda_device_code:{sys.argv[4]},dataset_arg:{dataset_arg}")
...
for epoch in range(n_epochs):
    ...
    if prior_trainable:
        ...
        with Session(service=service, backend=backend,max_time=3600) as session:
            ...
            sampler.options.error_mitigation = {"method": "zne"}
            # if epoch >=1:
            #     result = prior.train_on_batch(generated_before.cpu().numpy(), probs.cpu().numpy(), sampler, backend, n_epochs_prior)
